...
class StateMachine:
    ...
    def process_line(self, line: str):
        if not (
            ...
            if line != "\n":
                raise ValueError(
                    f"Wrong input. Line {line.__repr__()} is unacceptable")
            else:
                return False
        ...
        if from_state not in self.states:
            self.states[from_state] = State(
                name=from_state, is_final=from_state[0] == "f")
        if to_state not in self.states:
            self.states[to_state] = State(
                name=to_state, is_final=to_state[0] == "f")
        if symbol:
            self.alphabet.add(match.group("symbol"))
        else:
            self.is_determined = False
        transition = Transition(
            self.states[from_state], symbol, self.states[to_state])
                ...
                for ancestor in state.ancestors:
                    ...
                    for transition in state.outgoing_transitions:
                        self.__add_transition(Transition(
                            ancestor, transition.symbol, transition.to_state))
                self.__delete_state(state)
        logger.info(
            "Started copying transitions from nodes followed by empty transition")
    ...
    def __determine_transitions(self):
        ...
        while any(
            [
                (state.is_sink and not state.is_final) or (
                    state.is_unreachable and not state.is_initial)
    ...
    def __process_subset(self, subset: tuple[State]):
        ...
        for letter in self.alphabet:
            ...
            self.__add_transition(
                Transition(
                    self.new_states[new_node_name], letter, self.new_states[dest_node_name]), True
            )
            logger.info(
                f"Processed letter {letter} for node {new_node_name}. Destination is {dest_node_name}")

    def __delete_transition(self, transition: Transition, is_new=False):
        if not is_new:
            self.states[transition.from_state.name].remove_transition(
                transition)
            if transition.to_state != transition.from_state:
                self.states[transition.to_state.name].remove_transition(
                    transition)
            self.transitions.remove(transition)
        else:
            self.new_states[transition.from_state.name].remove_transition(
                transition)
            if transition.to_state != transition.from_state:
                self.new_states[transition.to_state.name].remove_transition(
                    transition)
    ...
    def __add_transition(self, transition: Transition, is_new=False) -> bool | None:
        ...
        if not is_new:
            result = self.states[transition.from_state.name].add_transition(
                transition, True)
            if result is not None:
                self.transitions.append(transition)
                if transition.to_state != transition.from_state:
                    self.states[transition.to_state.name].add_transition(
                        transition, False)
        else:
            result = self.new_states[transition.from_state.name].add_transition(
                transition, True)
            if result is not None:
                self.new_transitions.append(transition)
                if transition.to_state != transition.from_state:
                    self.new_states[transition.to_state.name].add_transition(
                        transition, False)
        return result
