...
class PushdownAutomata:
    ...
    def __init__(self, transitions: dict[str, set[str]] = None, string_to_process: str = None):
        ...
        logger.info(
            f"PDA created, {self.transitions=} {self.__string_to_process=}")
    ...
    def string_to_process(self, value: str | list[str]):
        if isinstance(value, list):
            ...
            raise TypeError(
                f"string_to_process cannot be of type {type(value)}")
    ...
    def add_transition(self, stack_symbol: str, new_states: set[str]):
        if stack_symbol in self.transitions:
            self.transitions[stack_symbol] = self.transitions[stack_symbol].union(
                new_states)
        else:
        ...
        while self.string_to_process and (self.stack or self.alternative_paths):
            ...
            if re.match("^[A-Z]$", stack_top):
                ...
                if stack_top == self.string_to_process[-1]:
                    ...
                    self.path.append(
                        f'consumed {self.string_to_process.pop()}')
                    continue
                    ...
                    logger.info(
                        "Didn't matched with terminal from string, and no restore point found")
                    break
